...
def main(url: str, width: int, height: int) -> None:
    ...
    try:
        ...
        result = driver.execute_script(r'''
          ...
          const info=n=>{if(!n)return null;const s=getComputedStyle(n);return {tag:n.tagName,cls:clean(n.className),rect:rect(n),text:clean(n.innerText||n.textContent).slice(0,100),display:s.display,fontFamily:s.fontFamily,fontSize:s.fontSize,fontWeight:s.fontWeight,line:s.lineHeight,pad:s.padding,margin:s.margin,gap:s.gap,bg:s.backgroundColor}};
          const roots=[...document.querySelectorAll('mio-toolbar,[class*=toolbar],[class*=mobile]')].filter(n=>rect(n)?.w>0).slice(0,12);
          const descendants=roots.flatMap(n=>[n,...n.querySelectorAll('*')]).filter(n=>rect(n)?.w>0).slice(0,80);
          return {viewport:{innerWidth,innerHeight},roots:roots.map(info),descendants:descendants.map(info)};
        ''')
